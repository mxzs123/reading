...
import { errorResponse, withApiError } from "@/lib/http";
...
export async function GET(request: NextRequest) {
  ...
  if (!word) {
    return errorResponse("缺少 word 参数", 400);
  }
  ...
  return withApiError(async () => {
    ...
    if (!response.ok) {
      return errorResponse("词典服务暂时不可用", response.status);
    }
...
function normalizeYoudaoResponse(raw: Record<string, unknown>): DictionaryData {
